...
![Tests](https://img.shields.io/badge/tests-292%20passing-16A34A)
...
- Accepts a **photo of the delivery note**: a reviewer uploads a photographed docket, a multimodal model reads what it confirms, and the *same* chat path turns it into an informal goods receipt — the image changes the input, not the trust, so the same ceiling and quantity checks apply and a photo never pays a large invoice on its own. When the photo is unclear the reading refuses rather than guesses.
...
pytest                                    # 292 offline tests, no API key needed
```
